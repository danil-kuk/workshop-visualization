import {
  CompetenceModel,
  KeyTechnologyStudentsModel,
  GraphModel, BubbleChartModel,
  KeyStatisticModel,
  TopCustomerModel,
} from 'src/features/diagrams/models'

export const fakeProjectCompetenciesStatistic: CompetenceModel[] = [
  {
    competency: 'базы данных',
    value: 3,
    competencyType: 'HardSkill',
  },
  {
    competency: 'machine learning',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'opencv',
    value: 7,
    competencyType: 'HardSkill',
  },
  {
    competency: 'dlib',
    value: 6,
    competencyType: 'HardSkill',
  },
  {
    competency: 'tensorflow',
    value: 5,
    competencyType: 'HardSkill',
  },
  {
    competency: 'keras',
    value: 7,
    competencyType: 'HardSkill',
  },
  {
    competency: 'html',
    value: 12,
    competencyType: 'HardSkill',
  },
  {
    competency: 'css',
    value: 11,
    competencyType: 'HardSkill',
  },
  {
    competency: 'php',
    value: 6,
    competencyType: 'HardSkill',
  },
  {
    competency: 'git',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'mysql',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'ios',
    value: 9,
    competencyType: 'HardSkill',
  },
  {
    competency: 'android',
    value: 9,
    competencyType: 'HardSkill',
  },
  {
    competency: 'статистика',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'математическое моделирование',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'react native',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'тестирование',
    value: 18,
    competencyType: 'HardSkill',
  },
  {
    competency: 'sql',
    value: 5,
    competencyType: 'HardSkill',
  },
  {
    competency: 'javascript',
    value: 6,
    competencyType: 'HardSkill',
  },
  {
    competency: 'анализ данных',
    value: 10,
    competencyType: 'HardSkill',
  },
  {
    competency: 'frontend',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'backend',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'аналитика данных',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'мобильная разработка',
    value: 8,
    competencyType: 'HardSkill',
  },
  {
    competency: 'java',
    value: 7,
    competencyType: 'HardSkill',
  },
  {
    competency: 'unity',
    value: 12,
    competencyType: 'HardSkill',
  },
  {
    competency: 'c#',
    value: 23,
    competencyType: 'HardSkill',
  },
  {
    competency: 'figma',
    value: 21,
    competencyType: 'HardSkill',
  },
  {
    competency: 'sketch',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'радиоэлектроника',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'python',
    value: 41,
    competencyType: 'HardSkill',
  },
  {
    competency: 'c++',
    value: 8,
    competencyType: 'HardSkill',
  },
  {
    competency: 'гис',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'ms sql',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'субд',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'wpf',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'level design',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'gitlab',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'github',
    value: 3,
    competencyType: 'HardSkill',
  },
  {
    competency: 'smm',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'stm32',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'arduino',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'js',
    value: 17,
    competencyType: 'HardSkill',
  },
  {
    competency: 'webgl',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'blender',
    value: 5,
    competencyType: 'HardSkill',
  },
  {
    competency: 'gimp',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'pytest',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'semantic web',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'linked data',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'rdf',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'rdfs',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'owl',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'sparql',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'big data архитектуры',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'apache spark',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'apache kafka',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'elk',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'influxdb',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'clickhouse',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'конвейеровы ci/cd',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'kubernates',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'docker swarm',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'linux',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'анализ экспрессии генов',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'continuous integration',
    value: 3,
    competencyType: 'HardSkill',
  },
  {
    competency: 'отвечать за качество результата',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'поддержка vk',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'instagram и youtube',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'detectron2',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'анализ данных continuous integration',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'ddd',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'postgresql',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'bpmn',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'usecase',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'angular',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'react',
    value: 5,
    competencyType: 'HardSkill',
  },
  {
    competency: 'json',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'шейп-графика',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'шейп-анимация',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'adobe after effects',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'уточнение требований',
    value: 2,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'распределение ролей',
    value: 2,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'постановка и контроль выполнения задач по графику',
    value: 2,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'организация процесса разработки',
    value: 2,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'manual testing',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'flutter',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'ionic',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'unity3d',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'тестирование webgl-сборок',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'qt',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'ms',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'postgress',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'netpdl',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'novelty search',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'построение гипотез',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'работа с даннымми',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'feature engineering',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'uml',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'общение с заказчиком',
    value: 6,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'выявление проблем и способы их решения',
    value: 4,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'составление документации',
    value: 4,
    competencyType: 'HardSkill',
  },
  {
    competency: 'матричные преобразования',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'отрисовка персонажей',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'базы даных',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'знание геометрии',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: '3d моделирование',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'ruby',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'rails',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'проектирование анимации',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'loraserver api',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'протокол mqtt',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'java-script',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'node-red',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'base64',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'telegram api',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'dialogflow',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'computer science',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'adobe animate',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'pandas',
    value: 3,
    competencyType: 'HardSkill',
  },
  {
    competency: 'geopandas',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'ms word',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'bitbucket',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'pycharm',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'flask',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'описание тест-кейсов',
    value: 4,
    competencyType: 'HardSkill',
  },
  {
    competency: '3d',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'коммуникативная',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'отслеживание выполнения задач во времени',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'корректировка проекта',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'с навыками программирования и организационными способностями',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'умение организовать процесс командной разработки',
    value: 4,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'figm',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'теория вероятности',
    value: 4,
    competencyType: 'HardSkill',
  },
  {
    competency: 'vue',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'tensoflow',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'entityframework',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'illustrator',
    value: 3,
    competencyType: 'HardSkill',
  },
  {
    competency: 'scss',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'прототипирование',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'asp',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'net core',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'составление отчета',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'навыки презентации',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'управление командой',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'web-программирование',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'balsamiq mockups',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'gitflow',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'компьютерная 3d-графика',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'линейная алгебра',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'школьная математика',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'pytorch',
    value: 7,
    competencyType: 'HardSkill',
  },
  {
    competency: 'javasctipt',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'ux-исследования',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'ux-аналитика',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'анимация интерфейсов',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'python web',
    value: 4,
    competencyType: 'HardSkill',
  },
  {
    competency: 'web-верстка',
    value: 15,
    competencyType: 'HardSkill',
  },
  {
    competency: 'с++',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'c++/qt',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'cad системы',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'электротехника',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'навык пайки',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'печатные платы',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'avr',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'простая механика',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'проектирование плат',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'основы электроники',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'inkscape',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'синтез игровых механик',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'ef core',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'базовые знания в машинном обучении',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'английский язык',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'adobe illustrator',
    value: 11,
    competencyType: 'HardSkill',
  },
  {
    competency: 'компас 3d',
    value: 10,
    competencyType: 'HardSkill',
  },
  {
    competency: 'сбор информации',
    value: 2,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'баг-трекинг',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'backend-разработка',
    value: 11,
    competencyType: 'HardSkill',
  },
  {
    competency: 'ui',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'бизнес анализ',
    value: 8,
    competencyType: 'HardSkill',
  },
  {
    competency: 'системный анализ',
    value: 8,
    competencyType: 'HardSkill',
  },
  {
    competency: 'дизайн',
    value: 10,
    competencyType: 'HardSkill',
  },
  {
    competency: 'vk',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'facebook',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'instagram',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'frontend-разработка',
    value: 10,
    competencyType: 'HardSkill',
  },
  {
    competency: 'работа с базами данных',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'nfc',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'скуд',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'эцп',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'шумометрия',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'r',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'c',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'микроконтроллеры',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'нейронные сети',
    value: 5,
    competencyType: 'HardSkill',
  },
  {
    competency: 'tinyml',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'глубокое обучение',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'физика процессов',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'написание документации',
    value: 5,
    competencyType: 'HardSkill',
  },
  {
    competency: 'когнитивные технологии',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'matplotlib',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'typescript',
    value: 3,
    competencyType: 'HardSkill',
  },
  {
    competency: 'математическая статистика',
    value: 8,
    competencyType: 'HardSkill',
  },
  {
    competency: 'nlp',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'теория графов',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'ux/ui',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'умение представлять результат исследования',
    value: 2,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'тестирование google',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'spss',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'amos',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'тестология',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'sem',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'творческое мышление',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'умение прогнозировать проблемы',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'laravel 5',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'mongodb',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'vue или svelte',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'контроль за графиком реализации проекта',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'знание английского языка',
    value: 5,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'умение читать техническую документацию и научные статьи',
    value: 5,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'ml',
    value: 3,
    competencyType: 'HardSkill',
  },
  {
    competency: 'фронтенд-фреймворки',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'ведение документации',
    value: 3,
    competencyType: 'HardSkill',
  },
  {
    competency: 'node js',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'photoshop',
    value: 6,
    competencyType: 'HardSkill',
  },
  {
    competency: 'node',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'nosql бд',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'разработка сервиса',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'разработка интерфейсов',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'дизайн системы; анализ данных',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'координация действий команды',
    value: 3,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'js web',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'adobe photoshop',
    value: 4,
    competencyType: 'HardSkill',
  },
  {
    competency: 'работа с документами',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'react-like frameworks',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'организация командного процесса',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'проверка выполнения поставленных задач',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'взаимодействие с куратором',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'ответственный за выполнение задач проекта в срок',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'ps',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'дизайн системы',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'анализ',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'разработка дизайна',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'разработка мероприятий',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'организация работы команды',
    value: 2,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'renpy',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'отрисовка фонов',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'распределение задач между командой',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'работа с растровой графикой',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'php 7',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'копирайт',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'запись скринкастов',
    value: 1,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'создание визуальных материалов',
    value: 1,
    competencyType: 'HardSkill',
  },
  {
    competency: 'net framework',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'windows forms (',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'net framework)',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 't-sql',
    value: 3,
    competencyType: 'HardSkill',
  },
  {
    competency: 'web',
    value: 3,
    competencyType: 'HardSkill',
  },
  {
    competency: 'навыки коммуникации',
    value: 3,
    competencyType: 'SoftSkill',
  },
  {
    competency: 'scikit-leran',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'основы алгоритмизации',
    value: 2,
    competencyType: 'HardSkill',
  },
  {
    competency: 'знание методик управления разработкой по',
    value: 3,
    competencyType: 'SoftSkill',
  },
]

export const fakeStudentCompetenciesStatistic: CompetenceModel[] = [
  {
    competency: 'базы данных',
    competencyType: 'HardSkill',
    value: 87,
  },
  {
    competency: 'machine learning',
    competencyType: 'HardSkill',
    value: 24,
  },
  {
    competency: 'opencv',
    competencyType: 'HardSkill',
    value: 8,
  },
  {
    competency: 'dlib',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'tensorflow',
    competencyType: 'HardSkill',
    value: 7,
  },
  {
    competency: 'keras',
    competencyType: 'HardSkill',
    value: 12,
  },
  {
    competency: 'html',
    competencyType: 'HardSkill',
    value: 157,
  },
  {
    competency: 'css',
    competencyType: 'HardSkill',
    value: 125,
  },
  {
    competency: 'php',
    competencyType: 'HardSkill',
    value: 28,
  },
  {
    competency: 'git',
    competencyType: 'HardSkill',
    value: 78,
  },
  {
    competency: 'mysql',
    competencyType: 'HardSkill',
    value: 33,
  },
  {
    competency: 'kotlin',
    competencyType: 'HardSkill',
    value: 12,
  },
  {
    competency: 'ios',
    competencyType: 'HardSkill',
    value: 6,
  },
  {
    competency: 'android',
    competencyType: 'HardSkill',
    value: 54,
  },
  {
    competency: 'deep learning',
    competencyType: 'HardSkill',
    value: 4,
  },
  {
    competency: 'статистика',
    competencyType: 'HardSkill',
    value: 18,
  },
  {
    competency: 'математическое моделирование',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'эконометрика',
    competencyType: 'HardSkill',
    value: 10,
  },
  {
    competency: 'тестирование по',
    competencyType: 'HardSkill',
    value: 23,
  },
  {
    competency: 'big data',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'flash-анимация',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'react native',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'тестирование',
    competencyType: 'HardSkill',
    value: 37,
  },
  {
    competency: 'sql',
    competencyType: 'HardSkill',
    value: 28,
  },
  {
    competency: 'postgesql',
    competencyType: 'HardSkill',
    value: 10,
  },
  {
    competency: 'интерфейсы',
    competencyType: 'HardSkill',
    value: 33,
  },
  {
    competency: 'project managment',
    competencyType: 'HardSkill',
    value: 33,
  },
  {
    competency: 'tilda',
    competencyType: 'HardSkill',
    value: 24,
  },
  {
    competency: 'javascript',
    competencyType: 'HardSkill',
    value: 48,
  },
  {
    competency: 'анализ данных',
    competencyType: 'HardSkill',
    value: 46,
  },
  {
    competency: 'frontend',
    competencyType: 'HardSkill',
    value: 39,
  },
  {
    competency: 'backend',
    competencyType: 'HardSkill',
    value: 62,
  },
  {
    competency: 'аналитика',
    competencyType: 'HardSkill',
    value: 72,
  },
  {
    competency: 'аналитика данных',
    competencyType: 'HardSkill',
    value: 37,
  },
  {
    competency: 'мобильная разработка',
    competencyType: 'HardSkill',
    value: 38,
  },
  {
    competency: 'java',
    competencyType: 'HardSkill',
    value: 58,
  },
  {
    competency: 'swift',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'xamarin',
    competencyType: 'HardSkill',
    value: 12,
  },
  {
    competency: '3d графика',
    competencyType: 'HardSkill',
    value: 17,
  },
  {
    competency: 'unity',
    competencyType: 'HardSkill',
    value: 67,
  },
  {
    competency: 'c#',
    competencyType: 'HardSkill',
    value: 312,
  },
  {
    competency: 'figma',
    competencyType: 'HardSkill',
    value: 81,
  },
  {
    competency: 'sketch',
    competencyType: 'HardSkill',
    value: 9,
  },
  {
    competency: 'радиоэлектроника',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'менеджер проектов',
    competencyType: 'HardSkill',
    value: 33,
  },
  {
    competency: 'python',
    competencyType: 'HardSkill',
    value: 151,
  },
  {
    competency: 'иностранные языки',
    competencyType: 'HardSkill',
    value: 51,
  },
  {
    competency: 'c++',
    competencyType: 'HardSkill',
    value: 31,
  },
  {
    competency: 'ar',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'vr',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'экономика',
    competencyType: 'HardSkill',
    value: 9,
  },
  {
    competency: 'физика',
    competencyType: 'HardSkill',
    value: 18,
  },
  {
    competency: 'autodesk',
    competencyType: 'HardSkill',
    value: 6,
  },
  {
    competency: 'ms sql',
    competencyType: 'HardSkill',
    value: 10,
  },
  {
    competency: 'субд',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'wpf',
    competencyType: 'HardSkill',
    value: 6,
  },
  {
    competency: 'ui design',
    competencyType: 'HardSkill',
    value: 36,
  },
  {
    competency: 'texturing',
    competencyType: 'HardSkill',
    value: 6,
  },
  {
    competency: 'animation',
    competencyType: 'HardSkill',
    value: 15,
  },
  {
    competency: 'drawing',
    competencyType: 'HardSkill',
    value: 29,
  },
  {
    competency: '3d modelling',
    competencyType: 'HardSkill',
    value: 16,
  },
  {
    competency: 'level design',
    competencyType: 'HardSkill',
    value: 11,
  },
  {
    competency: 'gamedev',
    competencyType: 'HardSkill',
    value: 36,
  },
  {
    competency: 'redis',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'cd',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'ci',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'gitlab',
    competencyType: 'HardSkill',
    value: 4,
  },
  {
    competency: 'github',
    competencyType: 'HardSkill',
    value: 47,
  },
  {
    competency: 'smm',
    competencyType: 'HardSkill',
    value: 12,
  },
  {
    competency: 'fusion360',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'stm32',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'arduino',
    competencyType: 'HardSkill',
    value: 9,
  },
  {
    competency: 'js',
    competencyType: 'HardSkill',
    value: 16,
  },
  {
    competency: 'django',
    competencyType: 'HardSkill',
    value: 9,
  },
  {
    competency: 'webgl',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'apple xcode',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'android sdk',
    competencyType: 'HardSkill',
    value: 9,
  },
  {
    competency: 'векторная графика',
    competencyType: 'HardSkill',
    value: 18,
  },
  {
    competency: '3ds max',
    competencyType: 'HardSkill',
    value: 8,
  },
  {
    competency: 'blender',
    competencyType: 'HardSkill',
    value: 27,
  },
  {
    competency: 'gimp',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'Коммуникабильность',
    competencyType: 'SoftSkill',
    value: 127,
  },
  {
    competency: 'Работа в команде',
    competencyType: 'SoftSkill',
    value: 174,
  },
  {
    competency: 'Ведение документации',
    competencyType: 'SoftSkill',
    value: 71,
  },
  {
    competency: 'конвейеровы ci/cd',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'docker swarm',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'linux',
    competencyType: 'HardSkill',
    value: 15,
  },
  {
    competency: 'continuous integration',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'отвечать за качество результата',
    competencyType: 'SoftSkill',
    value: 12,
  },
  {
    competency: 'поддержка vk',
    competencyType: 'HardSkill',
    value: 4,
  },
  {
    competency: 'instagram и youtube',
    competencyType: 'HardSkill',
    value: 15,
  },
  {
    competency: 'ddd',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'postgresql',
    competencyType: 'HardSkill',
    value: 4,
  },
  {
    competency: 'usecase',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'react',
    competencyType: 'HardSkill',
    value: 10,
  },
  {
    competency: 'json',
    competencyType: 'HardSkill',
    value: 7,
  },
  {
    competency: 'шейп-графика',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'шейп-анимация',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'adobe after effects',
    competencyType: 'HardSkill',
    value: 13,
  },
  {
    competency: 'уточнение требований',
    competencyType: 'SoftSkill',
    value: 11,
  },
  {
    competency: 'распределение ролей',
    competencyType: 'SoftSkill',
    value: 12,
  },
  {
    competency: 'постановка и контроль выполнения задач по графику',
    competencyType: 'SoftSkill',
    value: 22,
  },
  {
    competency: 'организация процесса разработки',
    competencyType: 'SoftSkill',
    value: 14,
  },
  {
    competency: 'manual testing',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'flutter',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'unity3d',
    competencyType: 'HardSkill',
    value: 16,
  },
  {
    competency: 'qt',
    competencyType: 'HardSkill',
    value: 10,
  },
  {
    competency: 'postgress',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'построение гипотез',
    competencyType: 'HardSkill',
    value: 4,
  },
  {
    competency: 'работа с даннымми',
    competencyType: 'HardSkill',
    value: 6,
  },
  {
    competency: 'uml',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'общение с заказчиком',
    competencyType: 'SoftSkill',
    value: 30,
  },
  {
    competency: 'выявление проблем и способы их решения',
    competencyType: 'SoftSkill',
    value: 27,
  },
  {
    competency: 'составление документации',
    competencyType: 'HardSkill',
    value: 23,
  },
  {
    competency: 'матричные преобразования',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'отрисовка персонажей',
    competencyType: 'HardSkill',
    value: 8,
  },
  {
    competency: 'наблюдение за работой группы',
    competencyType: 'HardSkill',
    value: 17,
  },
  {
    competency: 'базы даных',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'знание геометрии',
    competencyType: 'HardSkill',
    value: 4,
  },
  {
    competency: '3d моделирование',
    competencyType: 'HardSkill',
    value: 8,
  },
  {
    competency: 'ruby',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'проектирование анимации',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'java-script',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'telegram api',
    competencyType: 'HardSkill',
    value: 9,
  },
  {
    competency: 'dialogflow',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'computer science',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'adobe animate',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'pandas',
    competencyType: 'HardSkill',
    value: 4,
  },
  {
    competency: 'ms word',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'pycharm',
    competencyType: 'HardSkill',
    value: 9,
  },
  {
    competency: 'описание тест-кейсов',
    competencyType: 'HardSkill',
    value: 4,
  },
  {
    competency: '3d',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'коммуникативная',
    competencyType: 'SoftSkill',
    value: 10,
  },
  {
    competency: 'знание английского языка (разговорный уровень)',
    competencyType: 'SoftSkill',
    value: 15,
  },
  {
    competency: 'отслеживание выполнения задач во времени',
    competencyType: 'SoftSkill',
    value: 11,
  },
  {
    competency: 'корректировка проекта',
    competencyType: 'SoftSkill',
    value: 2,
  },
  {
    competency: 'умение организовать процесс командной разработки',
    competencyType: 'SoftSkill',
    value: 16,
  },
  {
    competency: 'figm',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'теория вероятности',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'vue',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'entityframework',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'illustrator',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'scss',
    competencyType: 'HardSkill',
    value: 4,
  },
  {
    competency: 'прототипирование',
    competencyType: 'HardSkill',
    value: 17,
  },
  {
    competency: 'asp',
    competencyType: 'HardSkill',
    value: 8,
  },
  {
    competency: 'net core',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'составление отчета',
    competencyType: 'SoftSkill',
    value: 18,
  },
  {
    competency: 'навыки презентации',
    competencyType: 'SoftSkill',
    value: 27,
  },
  {
    competency: 'управление командой',
    competencyType: 'SoftSkill',
    value: 18,
  },
  {
    competency: 'web-программирование',
    competencyType: 'HardSkill',
    value: 18,
  },
  {
    competency: 'gitflow',
    competencyType: 'HardSkill',
    value: 4,
  },
  {
    competency: 'компьютерная 3d-графика',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'линейная алгебра',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'школьная математика',
    competencyType: 'HardSkill',
    value: 27,
  },
  {
    competency: 'pytorch',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'javasctipt',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'ux-исследования',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'ux-аналитика',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'анимация интерфейсов',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'python web',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'web-верстка',
    competencyType: 'HardSkill',
    value: 13,
  },
  {
    competency: 'с++',
    competencyType: 'HardSkill',
    value: 4,
  },
  {
    competency: 'c++/qt',
    competencyType: 'HardSkill',
    value: 8,
  },
  {
    competency: 'навык пайки',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'печатные платы',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'avr',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'простая механика',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'проектирование плат',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'основы электроники',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'inkscape',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'синтез игровых механик',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'unity editor',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'unityengine',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'ef core',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'базовые знания в машинном обучении',
    competencyType: 'HardSkill',
    value: 4,
  },
  {
    competency: 'английский язык',
    competencyType: 'SoftSkill',
    value: 35,
  },
  {
    competency: 'adobe illustrator',
    competencyType: 'HardSkill',
    value: 27,
  },
  {
    competency: 'компас 3d',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'сбор информации',
    competencyType: 'SoftSkill',
    value: 6,
  },
  {
    competency: 'баг-трекинг',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'backend-разработка',
    competencyType: 'HardSkill',
    value: 13,
  },
  {
    competency: 'ui',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'бизнес анализ',
    competencyType: 'HardSkill',
    value: 4,
  },
  {
    competency: 'системный анализ',
    competencyType: 'HardSkill',
    value: 6,
  },
  {
    competency: 'дизайн',
    competencyType: 'HardSkill',
    value: 45,
  },
  {
    competency: 'vk',
    competencyType: 'HardSkill',
    value: 11,
  },
  {
    competency: 'instagram',
    competencyType: 'HardSkill',
    value: 15,
  },
  {
    competency: 'frontend-разработка',
    competencyType: 'HardSkill',
    value: 12,
  },
  {
    competency: 'работа с базами данных',
    competencyType: 'HardSkill',
    value: 7,
  },
  {
    competency: 'скуд',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'r',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'c',
    competencyType: 'HardSkill',
    value: 6,
  },
  {
    competency: 'нейронные сети',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'написание документации',
    competencyType: 'HardSkill',
    value: 9,
  },
  {
    competency: 'matplotlib',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'typescript',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'математическая статистика',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'nlp',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'теория графов',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'ux/ui',
    competencyType: 'HardSkill',
    value: 12,
  },
  {
    competency: 'умение представлять результат исследования',
    competencyType: 'SoftSkill',
    value: 4,
  },
  {
    competency: 'навык научного обобщения и обсуждения результатов знание англ',
    competencyType: 'SoftSkill',
    value: 1,
  },
  {
    competency: 'творческое мышление',
    competencyType: 'SoftSkill',
    value: 31,
  },
  {
    competency: 'умение прогнозировать проблемы',
    competencyType: 'SoftSkill',
    value: 11,
  },
  {
    competency: 'mongodb',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'vue или svelte',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'контроль за графиком реализации проекта',
    competencyType: 'SoftSkill',
    value: 5,
  },
  {
    competency: 'знание английского языка',
    competencyType: 'SoftSkill',
    value: 43,
  },
  {
    competency: 'умение читать техническую документацию и научные статьи',
    competencyType: 'SoftSkill',
    value: 13,
  },
  {
    competency: 'ml',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'фронтенд-фреймворки',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'ведение документации',
    competencyType: 'HardSkill',
    value: 15,
  },
  {
    competency: 'node js',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'photoshop',
    competencyType: 'HardSkill',
    value: 39,
  },
  {
    competency: 'node',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'nosql бд',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'разработка сервиса',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'разработка интерфейсов',
    competencyType: 'HardSkill',
    value: 8,
  },
  {
    competency: 'дизайн системы; анализ данных',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'координация действий команды',
    competencyType: 'SoftSkill',
    value: 17,
  },
  {
    competency: 'js web',
    competencyType: 'HardSkill',
    value: 6,
  },
  {
    competency: 'adobe photoshop',
    competencyType: 'HardSkill',
    value: 52,
  },
  {
    competency: 'работа с документами',
    competencyType: 'HardSkill',
    value: 22,
  },
  {
    competency: 'react-like frameworks',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'организация командного процесса',
    competencyType: 'SoftSkill',
    value: 18,
  },
  {
    competency: 'проверка выполнения поставленных задач',
    competencyType: 'SoftSkill',
    value: 15,
  },
  {
    competency: 'взаимодействие с куратором',
    competencyType: 'SoftSkill',
    value: 37,
  },
  {
    competency: 'ответственный за выполнение задач проекта в срок',
    competencyType: 'SoftSkill',
    value: 13,
  },
  {
    competency: 'ps',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'дизайн системы',
    competencyType: 'HardSkill',
    value: 12,
  },
  {
    competency: 'анализ',
    competencyType: 'HardSkill',
    value: 10,
  },
  {
    competency: 'разработка дизайна',
    competencyType: 'HardSkill',
    value: 41,
  },
  {
    competency: 'разработка мероприятий',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'организация работы команды',
    competencyType: 'SoftSkill',
    value: 32,
  },
  {
    competency: 'renpy',
    competencyType: 'HardSkill',
    value: 5,
  },
  {
    competency: 'отрисовка фонов',
    competencyType: 'HardSkill',
    value: 10,
  },
  {
    competency: 'распределение задач между командой',
    competencyType: 'SoftSkill',
    value: 38,
  },
  {
    competency: 'работа с растровой графикой',
    competencyType: 'HardSkill',
    value: 16,
  },
  {
    competency: 'php 7',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'копирайт',
    competencyType: 'SoftSkill',
    value: 2,
  },
  {
    competency: 'создание визуальных материалов',
    competencyType: 'HardSkill',
    value: 2,
  },
  {
    competency: 'windows forms (',
    competencyType: 'HardSkill',
    value: 3,
  },
  {
    competency: 'web',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'навыки коммуникации',
    competencyType: 'SoftSkill',
    value: 3,
  },
  {
    competency: 'scikit-leran',
    competencyType: 'HardSkill',
    value: 1,
  },
  {
    competency: 'основы алгоритмизации',
    competencyType: 'HardSkill',
    value: 1,
  },
]

export const fakeKeyTechnologyStudentsStatistic : KeyTechnologyStudentsModel[] = [
  {
    keyTechnology: 'Mobile',
    value: 39,
  },
  {
    keyTechnology: 'Чат-бот',
    value: 21,
  },
  {
    keyTechnology: 'Информационные системы',
    value: 38,
  },
  {
    keyTechnology: 'Web-разработка',
    value: 99,
  },
  {
    keyTechnology: 'Machine Learning',
    value: 59,
  },
  {
    keyTechnology: 'AR/VR',
    value: 18,
  },
  {
    keyTechnology: 'Математическое моделирование',
    value: 95,
  },
  {
    keyTechnology: 'GameDev',
    value: 199,
  },
  {
    keyTechnology: 'Робототехника',
    value: 6,
  },
  {
    keyTechnology: 'SMM',
    value: 16,
  },
  {
    keyTechnology: '3D-моделирование',
    value: 16,
  },
  {
    keyTechnology: 'Application',
    value: 18,
  },
  {
    keyTechnology: 'Web/SMM',
    value: 28,
  },
  {
    keyTechnology: 'Образование',
    value: 6,
  },
  {
    keyTechnology: 'Продолжение',
    value: 37,
  },
]

export const fakeKeyTechnologyCuratorsStatistic : GraphModel = {
  nodes: [
    {
      id: 1,
      label: '3D-моделирование',
      group: 'keyTech',
      value: 6,
    },
    {
      id: 2,
      label: 'Application',
      group: 'keyTech',
      value: 6,
    },
    {
      id: 3,
      label: 'AR/VR',
      group: 'keyTech',
      value: 3,
    },
    {
      id: 4,
      label: 'GameDev',
      group: 'keyTech',
      value: 19,
    },
    {
      id: 5,
      label: 'Machine Learning',
      group: 'keyTech',
      value: 20,
    },
    {
      id: 6,
      label: 'Mobile',
      group: 'keyTech',
      value: 11,
    },
    {
      id: 7,
      label: 'SMM',
      group: 'keyTech',
      value: 2,
    },
    {
      id: 8,
      label: 'Web/SMM',
      group: 'keyTech',
      value: 8,
    },
    {
      id: 9,
      label: 'Web-разработка',
      group: 'keyTech',
      value: 14,
    },
    {
      id: 10,
      label: 'Информационные системы',
      group: 'keyTech',
      value: 13,
    },
    {
      id: 11,
      label: 'Математическое моделирование',
      group: 'keyTech',
      value: 25,
    },
    {
      id: 14,
      label: 'Робототехника',
      group: 'keyTech',
      value: 2,
    },
    {
      id: 15,
      label: 'Чат-бот',
      group: 'keyTech',
      value: 2,
    },

    {
      id: 16,
      label: 'Акулич Сергей Андреевич',
      group: 'curator',
      value: 4,
    },
    {
      id: 17,
      label: 'Блохин Олег Анатольевич',
      group: 'curator',
      value: 2,
    },
    {
      id: 18,
      label: 'Ботов Михаил Алексеевич',
      group: 'curator',
      value: 14,
    },
    {
      id: 19,
      label: 'Васькин Павел Ильич',
      group: 'curator',
      value: 2,
    },
    {
      id: 20,
      label: 'Евсегнеев Олег Анатольевич',
      group: 'curator',
      value: 4,
    },
    {
      id: 21,
      label: 'Изотов Илья Николаевич',
      group: 'curator',
      value: 2,
    },
    {
      id: 22,
      label: 'Комаров Денис Евгеньевич',
      group: 'curator',
      value: 2,
    },
    {
      id: 23,
      label: 'Комоцкий Евгений Игоревич',
      group: 'curator',
      value: 3,
    },
    {
      id: 24,
      label: 'Куклин Илья Эдуардович',
      group: 'curator',
      value: 9,
    },
    {
      id: 25,
      label: 'Лимановская Оксана Викторовна',
      group: 'curator',
      value: 15,
    },
    {
      id: 26,
      label: 'Протасов Денис Николаевич',
      group: 'curator',
      value: 5,
    },
    {
      id: 27,
      label: 'Сальников Виктор Борисович',
      group: 'curator',
      value: 2,
    },
    {
      id: 28,
      label: 'Стародубцев Илья Сергеевич',
      group: 'curator',
      value: 4,
    },
    {
      id: 29,
      label: 'Ушенин Константин Сергеевич',
      group: 'curator',
      value: 5,
    },
    {
      id: 30,
      label: 'Хлебников Николай Александрович',
      group: 'curator',
      value: 3,
    },
    {
      id: 31,
      label: 'Чикова Ольга Анатольевна',
      group: 'curator',
      value: 3,
    },
    {
      id: 32,
      label: 'Шадрин Денис Борисович',
      group: 'curator',
      value: 8,
    },
    {
      id: 33,
      label: 'Шестеров Михаил Андреевич',
      group: 'curator',
      value: 24,
    },
    {
      id: 34,
      label: 'Шибаев Вячеслав Алексеевич',
      group: 'curator',
      value: 2,
    },
  ],
  edges: [
    {
      from: 16,
      to: 9,
      value: 4,
    },
    {
      from: 17,
      to: 5,
      value: 1,
    },
    {
      from: 17,
      to: 11,
      value: 1,
    },
    {
      from: 18,
      to: 15,
      value: 1,
    },
    {
      from: 18,
      to: 10,
      value: 1,
    },
    {
      from: 18,
      to: 5,
      value: 3,
    },
    {
      from: 18,
      to: 11,
      value: 4,
    },
    {
      from: 18,
      to: 4,
      value: 5,
    },
    {
      from: 19,
      to: 11,
      value: 2,
    },
    {
      from: 20,
      to: 4,
      value: 2,
    },
    {
      from: 20,
      to: 14,
      value: 2,
    },
    {
      from: 21,
      to: 6,
      value: 1,
    },
    {
      from: 21,
      to: 8,
      value: 1,
    },
    {
      from: 22,
      to: 5,
      value: 1,
    },
    {
      from: 22,
      to: 11,
      value: 1,
    },
    {
      from: 23,
      to: 5,
      value: 2,
    },
    {
      from: 23,
      to: 2,
      value: 1,
    },
    {
      from: 24,
      to: 6,
      value: 1,
    },
    {
      from: 24,
      to: 15,
      value: 1,
    },
    {
      from: 24,
      to: 10,
      value: 1,
    },
    {
      from: 24,
      to: 9,
      value: 2,
    },
    {
      from: 24,
      to: 5,
      value: 1,
    },
    {
      from: 24,
      to: 11,
      value: 2,
    },
    {
      from: 24,
      to: 4,
      value: 1,
    },
    {
      from: 25,
      to: 6,
      value: 1,
    },
    {
      from: 25,
      to: 10,
      value: 4,
    },
    {
      from: 25,
      to: 9,
      value: 1,
    },
    {
      from: 25,
      to: 11,
      value: 6,
    },
    {
      from: 25,
      to: 4,
      value: 1,
    },
    {
      from: 25,
      to: 1,
      value: 2,
    },
    {
      from: 26,
      to: 5,
      value: 4,
    },
    {
      from: 26,
      to: 1,
      value: 1,
    },
    {
      from: 27,
      to: 11,
      value: 2,
    },
    {
      from: 28,
      to: 10,
      value: 1,
    },
    {
      from: 28,
      to: 3,
      value: 1,
    },
    {
      from: 28,
      to: 11,
      value: 1,
    },
    {
      from: 28,
      to: 4,
      value: 1,
    },
    {
      from: 29,
      to: 10,
      value: 1,
    },
    {
      from: 29,
      to: 5,
      value: 2,
    },
    {
      from: 29,
      to: 11,
      value: 1,
    },
    {
      from: 29,
      to: 7,
      value: 1,
    },
    {
      from: 30,
      to: 10,
      value: 1,
    },
    {
      from: 30,
      to: 11,
      value: 1,
    },
    {
      from: 30,
      to: 8,
      value: 1,
    },
    {
      from: 31,
      to: 3,
      value: 1,
    },
    {
      from: 31,
      to: 11,
      value: 2,
    },
    {
      from: 32,
      to: 9,
      value: 2,
    },
    {
      from: 32,
      to: 11,
      value: 2,
    },
    {
      from: 32,
      to: 4,
      value: 3,
    },
    {
      from: 32,
      to: 7,
      value: 1,
    },
    {
      from: 33,
      to: 6,
      value: 8,
    },
    {
      from: 33,
      to: 10,
      value: 2,
    },
    {
      from: 33,
      to: 9,
      value: 2,
    },
    {
      from: 33,
      to: 4,
      value: 4,
    },
    {
      from: 33,
      to: 1,
      value: 1,
    },
    {
      from: 33,
      to: 2,
      value: 3,
    },
    {
      from: 33,
      to: 8,
      value: 4,
    },
    {
      from: 34,
      to: 5,
      value: 2,
    },
  ],
}

export const fakeKeyTechnologyConsumersStatistic : GraphModel = {
  nodes: [
    {
      id: 1,
      label: '3D-моделирование',
      group: 'keyTech',
      value: 6,
    },
    {
      id: 2,
      label: 'Application',
      group: 'keyTech',
      value: 6,
    },
    {
      id: 3,
      label: 'AR/VR',
      group: 'keyTech',
      value: 3,
    },
    {
      id: 4,
      label: 'GameDev',
      group: 'keyTech',
      value: 19,
    },
    {
      id: 5,
      label: 'Machine Learning',
      group: 'keyTech',
      value: 20,
    },
    {
      id: 6,
      label: 'Mobile',
      group: 'keyTech',
      value: 11,
    },
    {
      id: 7,
      label: 'SMM',
      group: 'keyTech',
      value: 2,
    },
    {
      id: 8,
      label: 'Web/SMM',
      group: 'keyTech',
      value: 8,
    },
    {
      id: 9,
      label: 'Web-разработка',
      group: 'keyTech',
      value: 14,
    },
    {
      id: 10,
      label: 'Информационные системы',
      group: 'keyTech',
      value: 13,
    },
    {
      id: 11,
      label: 'Математическое моделирование',
      group: 'keyTech',
      value: 25,
    },
    {
      id: 14,
      label: 'Робототехника',
      group: 'keyTech',
      value: 2,
    },
    {
      id: 15,
      label: 'Чат-бот',
      group: 'keyTech',
      value: 2,
    },

    {
      id: 16,
      label: 'PXL PRO (ООО «Финист-АйТи»',
      group: 'consumer',
      value: 2,
    },
    {
      id: 17,
      label: 'АО «Альфа-Банк»',
      group: 'consumer',
      value: 8,
    },
    {
      id: 18,
      label: 'АО «Научно-производственное объединение автоматики имени академика Н. А. Семихатова»',
      group: 'consumer',
      value: 5,
    },
    {
      id: 19,
      label: 'ЗАО «Морские компьютерные системы»',
      group: 'consumer',
      value: 2,
    },
    {
      id: 20,
      label: 'Институт иммунологии и физиологии УрО РАН',
      group: 'consumer',
      value: 2,
    },
    {
      id: 21,
      label: 'Институт проектирования, архитектуры и дизайна (ИнПАД)',
      group: 'consumer',
      value: 2,
    },
    {
      id: 22,
      label: 'ИРИТ-РТФ',
      group: 'consumer',
      value: 8,
    },
    {
      id: 23,
      label: 'каф. ТФ, ФТИ',
      group: 'consumer',
      value: 4,
    },
    {
      id: 24,
      label: 'Научно-Производственное Объединение КАРАТ',
      group: 'consumer',
      value: 2,
    },
    {
      id: 25,
      label: 'Образовательная платформа «Юрайт»',
      group: 'consumer',
      value: 3,
    },
    {
      id: 26,
      label: 'Общество с ограниченной ответственностью «66 Бит»',
      group: 'consumer',
      value: 2,
    },
    {
      id: 27,
      label: 'ООО «КОПТИС»',
      group: 'consumer',
      value: 2,
    },
    {
      id: 28,
      label: 'ООО «УЦСБ»',
      group: 'consumer',
      value: 5,
    },
    {
      id: 29,
      label: 'ООО Лаборатория кода, город Пермь',
      group: 'consumer',
      value: 2,
    },
    {
      id: 30,
      label: 'ПАО «МегаФон»',
      group: 'consumer',
      value: 4,
    },
    {
      id: 31,
      label: 'СКБ-ЛАБ',
      group: 'consumer',
      value: 4,
    },
    {
      id: 32,
      label: 'Уральский государственный педагогический университет',
      group: 'consumer',
      value: 3,
    },
    {
      id: 33,
      label: 'Уральский федеральный университет',
      group: 'consumer',
      value: 3,
    },
    {
      id: 34,
      label: 'УрФУ',
      group: 'consumer',
      value: 27,
    },
    {
      id: 35,
      label: 'УрФУ Институт Фундаментального образования',
      group: 'consumer',
      value: 5,
    },
    {
      id: 36,
      label: 'УрФУ, департамент психологии; Институт математики и механики УрО РАН',
      group: 'consumer',
      value: 3,
    },
    {
      id: 37,
      label: 'ФГБОУ ВО УГМУ «Уральский государственный медицинский университет» Минздрава России, кафедра терапевтической стоматологии и пропедевтики стоматологических заболеваний',
      group: 'consumer',
      value: 2,
    },
    {
      id: 38,
      label: 'ФГБОУ ВО Уральский государственный медицинский университет Минздрава России',
      group: 'consumer',
      value: 2,
    },
    {
      id: 39,
      label: 'ФГБОУ ВО УГМУ Минздрава России',
      group: 'consumer',
      value: 2,
    },
    {
      id: 40,
      label: 'ФГБОУ ВО Уральский государственный медицинский университет минздрава России.',
      group: 'consumer',
      value: 4,
    },
  ],
  edges: [
    {
      from: 16,
      to: 6,
      value: 1,
    },
    {
      from: 16,
      to: 9,
      value: 1,
    },
    {
      from: 17,
      to: 6,
      value: 6,
    },
    {
      from: 17,
      to: 10,
      value: 1,
    },
    {
      from: 17,
      to: 9,
      value: 1,
    },
    {
      from: 18,
      to: 5,
      value: 4,
    },
    {
      from: 18,
      to: 1,
      value: 1,
    },
    {
      from: 19,
      to: 11,
      value: 2,
    },
    {
      from: 20,
      to: 10,
      value: 1,
    },
    {
      from: 20,
      to: 11,
      value: 1,
    },
    {
      from: 21,
      to: 11,
      value: 2,
    },
    {
      from: 22,
      to: 9,
      value: 2,
    },
    {
      from: 22,
      to: 11,
      value: 2,
    },
    {
      from: 22,
      to: 4,
      value: 3,
    },
    {
      from: 22,
      to: 7,
      value: 1,
    },
    {
      from: 23,
      to: 4,
      value: 2,
    },
    {
      from: 23,
      to: 14,
      value: 2,
    },
    {
      from: 24,
      to: 6,
      value: 1,
    },
    {
      from: 24,
      to: 8,
      value: 1,
    },
    {
      from: 25,
      to: 10,
      value: 1,
    },
    {
      from: 25,
      to: 11,
      value: 1,
    },
    {
      from: 25,
      to: 8,
      value: 1,
    },
    {
      from: 26,
      to: 9,
      value: 1,
    },
    {
      from: 26,
      to: 5,
      value: 1,
    },
    {
      from: 27,
      to: 5,
      value: 1,
    },
    {
      from: 27,
      to: 11,
      value: 1,
    },
    {
      from: 28,
      to: 10,
      value: 2,
    },
    {
      from: 28,
      to: 5,
      value: 2,
    },
    {
      from: 28,
      to: 11,
      value: 1,
    },
    {
      from: 29,
      to: 5,
      value: 2,
    },
    {
      from: 30,
      to: 15,
      value: 1,
    },
    {
      from: 30,
      to: 10,
      value: 1,
    },
    {
      from: 30,
      to: 5,
      value: 1,
    },
    {
      from: 30,
      to: 11,
      value: 1,
    },
    {
      from: 31,
      to: 9,
      value: 4,
    },
    {
      from: 32,
      to: 3,
      value: 1,
    },
    {
      from: 32,
      to: 11,
      value: 2,
    },
    {
      from: 33,
      to: 9,
      value: 1,
    },
    {
      from: 33,
      to: 11,
      value: 1,
    },
    {
      from: 33,
      to: 7,
      value: 1,
    },
    {
      from: 34,
      to: 10,
      value: 1,
    },
    {
      from: 34,
      to: 9,
      value: 1,
    },
    {
      from: 34,
      to: 5,
      value: 5,
    },
    {
      from: 34,
      to: 11,
      value: 4,
    },
    {
      from: 34,
      to: 4,
      value: 5,
    },
    {
      from: 34,
      to: 2,
      value: 1,
    },
    {
      from: 35,
      to: 2,
      value: 1,
    },
    {
      from: 35,
      to: 8,
      value: 4,
    },
    {
      from: 36,
      to: 10,
      value: 1,
    },
    {
      from: 36,
      to: 3,
      value: 2,
    },
    {
      from: 37,
      to: 9,
      value: 1,
    },
    {
      from: 37,
      to: 4,
      value: 1,
    },
    {
      from: 38,
      to: 11,
      value: 1,
    },
    {
      from: 38,
      to: 1,
      value: 1,
    },
    {
      from: 39,
      to: 11,
      value: 1,
    },
    {
      from: 39,
      to: 1,
      value: 1,
    },
    {
      from: 40,
      to: 6,
      value: 1,
    },
    {
      from: 40,
      to: 10,
      value: 2,
    },
    {
      from: 40,
      to: 11,
      value: 1,
    },
  ],
}

export const fakeProjectDistributionStatistic : BubbleChartModel = {
  name: 'Проектный практикум',
  children: [
    {
      name: 'Профессиональный',
      children: [
        {
          name: 'Mobile',
          children: [
            {
              name: 'Online площадка для online связи, собраний, встреч.',
              children: [
                {
                  name: 'eh13',
                  value: 2,
                },
              ],
            },
            {
              name: 'Приложение по электронным пропускам',
              children: [
                {
                  name: 'SessionStart',
                  value: 5,
                },
              ],
            },
            {
              name: 'Управление устройством с использованием когнитивных технологий',
              children: [
                {
                  name: 'MoveUP',
                  value: 5,
                },
              ],
            },
            {
              name: 'Оплата товаров/услуг ФЛ в адрес ЮЛ с использование технологии NFC',
              children: [
                {
                  name: 'CatStack',
                  value: 3,
                },
              ],
            },
            {
              name: 'Автоматизация формирования электронной визитки для ФЛ',
              children: [
                {
                  name: 'Внештатные альфы',
                  value: 4,
                },
              ],
            },
            {
              name: 'ПО для обмена данными с портативным устройством мониторинга уровня и накопленной дозы ультрафиолетового излучения (УФИ)',
              children: [
                {
                  name: 'SimplEffect',
                  value: 2,
                },
              ],
            },
          ],
        },
        {
          name: 'Чат-бот',
          children: [
            {
              name: 'Бот помогающий учёту рабочего времени',
              children: [
                {
                  name: 'Lion\'s_Victory_23',
                  value: 4,
                },
                {
                  name: 'Крокодилы',
                  value: 4,
                },
              ],
            },
            {
              name: 'Чат-бот для экономических игр',
              children: [
                {
                  name: 'SimpleEconomy',
                  value: 5,
                },
              ],
            },
          ],
        },
        {
          name: 'Информационные системы',
          children: [
            {
              name: 'Виртуальный пожар/инфекция',
              children: [
                {
                  name: 'Пожарники',
                  value: 3,
                },
              ],
            },
            {
              name: 'Платформа образовательных игр',
              children: [
                {
                  name: 'Salyanka Team',
                  value: 4,
                },
              ],
            },
            {
              name: 'Рекомендательная система учебных материалов',
              children: [
                {
                  name: 'SNMTNR',
                  value: 3,
                },
                {
                  name: '2pizza',
                  value: 2,
                },
              ],
            },
          ],
        },
        {
          name: 'Web-разработка',
          children: [
            {
              name: 'Виртуальная ЭЦП',
              children: [
                {
                  name: '10',
                  value: 5,
                },
              ],
            },
            {
              name: 'ПДД GO',
              children: [
                {
                  name: 'CROC',
                  value: 2,
                },
                {
                  name: 'Чёткая группа',
                  value: 3,
                },
              ],
            },
            {
              name: 'Модернизация сайта ИРИТ-РТФ',
              children: [
                {
                  name: 'Молодцы 3.0',
                  value: 4,
                },
              ],
            },
            {
              name: 'Модернизация сайта ИнФО',
              children: [
                {
                  name: 'BIG SVM',
                  value: 9,
                },
              ],
            },
            {
              name: 'Web-сайт «Экономика Региона»',
              children: [
                {
                  name: 'Феникс',
                  value: 3,
                },
              ],
            },
            {
              name: 'Мониторинг сервисов оценки приложение AppStore/PlayMarket для интеграций статистики',
              children: [
                {
                  name: 'Империум',
                  value: 3,
                },
              ],
            },
            {
              name: 'Приложение планер стоматологического здоровья для фитнесс трекеров',
              children: [
                {
                  name: 'Gaming 35',
                  value: 3,
                },
              ],
            },
            {
              name: 'Система для стажировок в компании',
              children: [
                {
                  name: 'Internaft',
                  value: 4,
                },
              ],
            },
            {
              name: 'Внутренний портал компании',
              children: [
                {
                  name: 'ASTRUM',
                  value: 4,
                },
              ],
            },
            {
              name: 'Доработка дизайна и пользовательского интерфейса сервиса ПроКомпетенции.',
              children: [
                {
                  name: 'GUINNESS',
                  value: 4,
                },
                {
                  name: 'САуНД',
                  value: 4,
                },
              ],
            },
          ],
        },
        {
          name: 'Machine Learning',
          children: [
            {
              name: 'Семантический анализ отзывов на онлайн-курсы',
              children: [
                {
                  name: 'Ну так получилось',
                  value: 4,
                },
              ],
            },
          ],
        },
        {
          name: 'AR/VR',
          children: [
            {
              name: 'Инструмент манипуляции',
              children: [
                {
                  name: 'ThreeXyNine и Миша',
                  value: 4,
                },
              ],
            },
            {
              name: 'Чат-бот для обучения школьников дисциплине «Индивидуальный проект» ( созданию простого VR-приложения для Android-устройста на Vuforia и Unity 3D)',
              children: [
                {
                  name: 'Колумбийские голуби',
                  value: 5,
                },
              ],
            },
          ],
        },
        {
          name: 'Математическое моделирование',
          children: [
            {
              name: 'Аналитика цифровых образовательных материалов',
              children: [
                {
                  name: 'Команда 31',
                  value: 4,
                },
              ],
            },
            {
              name: 'Графика знаний',
              children: [
                {
                  name: 'Awesome',
                  value: 5,
                },
                {
                  name: 'Модели математические',
                  value: 3,
                },
              ],
            },
            {
              name: 'Динамическая модель компетенции',
              children: [
                {
                  name: 'Seven',
                  value: 5,
                },
                {
                  name: 'судьбы',
                  value: 5,
                },
              ],
            },
            {
              name: 'Система для создания и совместного редактирования больших графов',
              children: [
                {
                  name: 'Power of Youth',
                  value: 6,
                },
              ],
            },
            {
              name: 'Имитационная модель судового трафика в Керченском проливе',
              children: [
                {
                  name: 'SAINO',
                  value: 5,
                },
              ],
            },
            {
              name: 'Аналитика и визуализация данных проектного семестра в сервисе ПроКомпетенции.',
              children: [
                {
                  name: 'Generics',
                  value: 4,
                },
                {
                  name: 'CXC',
                  value: 6,
                },
              ],
            },
            {
              name: 'Анализ результатов автотестов',
              children: [
                {
                  name: 'Star_Dust Crusaders',
                  value: 2,
                },
              ],
            },
            {
              name: 'Предиктивная модель успеваемости студента',
              children: [
                {
                  name: 'MoonWolker4s',
                  value: 2,
                },
              ],
            },
          ],
        },
        {
          name: 'GameDev',
          children: [
            {
              name: 'Геймдизайн в математике',
              children: [
                {
                  name: 'P.T.V.E',
                  value: 4,
                },
                {
                  name: 'Dickoobrazes',
                  value: 5,
                },
                {
                  name: 'Одержимый поребрик',
                  value: 5,
                },
                {
                  name: 'beCookie',
                  value: 5,
                },
              ],
            },
            {
              name: 'Компьютерная 2D игра на свободную тему',
              children: [
                {
                  name: 'California Apostles',
                  value: 5,
                },
              ],
            },
            {
              name: 'GameDev. Разработка контаента для курса по разработке игр.',
              children: [
                {
                  name: 'Glow Glob',
                  value: 4,
                },
              ],
            },
            {
              name: 'Игровая графика',
              children: [
                {
                  name: '№7',
                  value: 5,
                },
                {
                  name: ', великая команда',
                  value: 5,
                },
                {
                  name: 'Тираннозавры',
                  value: 5,
                },
                {
                  name: 'Se7en',
                  value: 7,
                },
              ],
            },
            {
              name: 'Разработка образовательных игр',
              children: [
                {
                  name: '-',
                  value: 2,
                },
                {
                  name: 'ARCH',
                  value: 4,
                },
                {
                  name: 'Double-AA',
                  value: 4,
                },
                {
                  name: 'игровые игроделы игор онлайн 2007, 2008,2009',
                  value: 5,
                },
              ],
            },
            {
              name: 'Разработка Unity assets',
              children: [
                {
                  name: 'Tech Stonks',
                  value: 5,
                },
                {
                  name: 'Team 144p',
                  value: 4,
                },
                {
                  name: 'NoTeamNameклуб закрытых закрыт и не отпускает',
                  value: 3,
                },
              ],
            },
            {
              name: 'Coding Learn',
              children: [
                {
                  name: 'Wish Granters',
                  value: 5,
                },
              ],
            },
            {
              name: 'Образовательная игра «Экспедиция в Арктику»',
              children: [
                {
                  name: 'Бездельники и тунеядцы',
                  value: 4,
                },
              ],
            },
            {
              name: 'Образовательные исторические квесты и игры в цифровой среде (прототип и общие принципы)',
              children: [
                {
                  name: 'питоний кусь',
                  value: 3,
                },
              ],
            },
            {
              name: 'Компьютерная игра аркада на стоматологическую тематику для школьников среднего звена',
              children: [
                {
                  name: 'Целый Байт',
                  value: 3,
                },
              ],
            },
          ],
        },
        {
          name: 'Робототехника',
          children: [
            {
              name: 'Роботизированный стенд для тестирования позвоночных эндопротезов',
              children: [
                {
                  name: 'WAW-DH',
                  value: 4,
                },
              ],
            },
          ],
        },
        {
          name: 'SMM',
          children: [
            {
              name: 'SMM ИРИТ-РТФ',
              children: [
                {
                  name: 'Туса-Джуса',
                  value: 5,
                },
              ],
            },
            {
              name: 'Научные коммуникации',
              children: [
                {
                  name: 'крутые',
                  value: 8,
                },
              ],
            },
          ],
        },
        {
          name: '3D-моделирование',
          children: [
            {
              name: 'Виртуальная 3d онлайн-лаборатория по физике',
              children: [
                {
                  name: 'Vertex',
                  value: 4,
                },
              ],
            },
          ],
        },
        {
          name: 'Application',
          children: [
            {
              name: 'Помоги Оленям - найди место под заповедник на Таймыре',
              children: [
                {
                  name: 'Сохатый',
                  value: 3,
                },
              ],
            },
          ],
        },
        {
          name: 'Web/SMM',
          children: [
            {
              name: 'Telegram-бот для Проектного практикума и курса Информационные технологии и сервисы',
              children: [
                {
                  name: 'Intelligency',
                  value: 5,
                },
              ],
            },
            {
              name: 'Продвижение онлайн курса Информационные технологии и сервисы и Проектного практикума',
              children: [
                {
                  name: '9',
                  value: 2,
                },
              ],
            },
            {
              name: 'Создание анимационного промо ролика онлайн курса Информационные технологии и сервисы',
              children: [
                {
                  name: 'могучие рейнджеры',
                  value: 4,
                },
              ],
            },
          ],
        },
      ],
    },
    {
      name: 'Научный',
      children: [
        {
          name: 'Machine Learning',
          children: [
            {
              name: 'Создание инструмента сегментации улично-дорожной сети на основании данных ортофотосъемки',
              children: [
                {
                  name: 'COPYRIGHT',
                  value: 3,
                },
              ],
            },
            {
              name: 'Разработка алгоритма выделения признаков объекта на фотографии',
              children: [
                {
                  name: 'Дружная команда',
                  value: 3,
                },
              ],
            },
            {
              name: 'Разработка алгоритма оценки характеристик движущегося автомобиля.',
              children: [
                {
                  name: 'ML 2.0',
                  value: 3,
                },
                {
                  name: 'Spice Girls',
                  value: 2,
                },
              ],
            },
            {
              name: 'Мониторинг эффективности регионов в борьбе с COVID-19',
              children: [
                {
                  name: 'Новочебоксарск',
                  value: 2,
                },
              ],
            },
          ],
        },
        {
          name: 'Математическое моделирование',
          children: [
            {
              name: 'Цифровизационный подход к измерению биологического возраста человека',
              children: [
                {
                  name: 'Клан Олигофренов',
                  value: 4,
                },
              ],
            },
          ],
        },
      ],
    },
    {
      name: 'Предпринимательский',
      children: [
        {
          name: 'Mobile',
          children: [
            {
              name: 'Мобильное приложение “Foody”',
              children: [
                {
                  name: 'Jingu Digital :: Foody',
                  value: 3,
                },
              ],
            },
          ],
        },
        {
          name: 'Web-разработка',
          children: [
            {
              name: 'Маркбук.рф',
              children: [
                {
                  name: 'команда настоящих профессионалов',
                  value: 5,
                },
              ],
            },
          ],
        },
        {
          name: 'GameDev',
          children: [
            {
              name: 'GameDev',
              children: [
                {
                  name: 'Бафомет',
                  value: 6,
                },
              ],
            },
            {
              name: 'Виртуальный музей игр и компьютерной графики',
              children: [
                {
                  name: 'Chepernul',
                  value: 5,
                },
              ],
            },
          ],
        },
        {
          name: '3D-моделирование',
          children: [
            {
              name: 'Видео-рендеринг 3D-сцен.',
              children: [
                {
                  name: '///',
                  value: 1,
                },
              ],
            },
          ],
        },
        {
          name: 'Application',
          children: [
            {
              name: 'Financial Calculator',
              children: [
                {
                  name: 'Finance Comics',
                  value: 6,
                },
              ],
            },
            {
              name: 'Органайзер',
              children: [
                {
                  name: 'Команда Б',
                  value: 3,
                },
              ],
            },
          ],
        },
        {
          name: 'Web/SMM',
          children: [
            {
              name: 'Wellbe - health coaching online',
              children: [
                {
                  name: 'Бафомет',
                  value: 4,
                },
              ],
            },
          ],
        },
      ],
    },
  ],
}

export const fakeProjectRolesTopStatistic = [
  'Аналитик',
  'Backend разработчик',
  'Дизайнер',
  'Тестировщик',
  'Участник проекта',
  'Frontend разработчик',
  'NoRoles',
  'Разработчик',
  'Участник команды',
  'Программист',
  'Разработчики - front-end',
  'Разработчик - back-end',
  'Тим-лидер',
  'Разработчик ML',
  'Геймдизайнер',
  'Программист C# / разработчик в Unity',
  '3d-моделист',
  'программист',
  'Тимлид',
  'Аналитик данных',
  'Художник',
  'Верстальщик',
  'Исследователь',
  'JS разработчик',
  'Unity-разработчик',
  'Менеджер',
  'Дизайнер игровой механики',
  'Геймплей-программист\nХудожник-иллюстратор',
  'Геймплей-программист',
  'Электроник',
  'Инженер',
  'Иллюстратор',
  'Аналитик UI бота',
  'TeamLeader',
  'Тим-лид',
  'SMM-специалист',
  'Гейм-дизайнер',
  'Организтор',
  'Тестолог',
  'Backend разработчик\nДизайнер',
  'специалист по Machine Learning',
  'Data Engineer',
  'Биоинформатик',
  'Backend-разработчик',
  'Разработчик баз данных',
  'DevOps',
  'Специалист по анализу данных',
  'Графический дизайн',
  'Тестирование',
  'Программное наполнение',
  'UX разработчик',
  'Backend разрабочик',
  'Аналитик данный',
  'Копирайтер',
  'Сценарист',
  'Разработчик Python',
  'Backend разработчик 2',
  'Backend разработчик 1',
  'Аналитики3',
  'Аналитики2',
  'Аналитик1',
  'Математик/Аналитик',
  'Дизайнер (опционально)',
  'специалист по интернет-рекламе',
  'Маркетолог',
  'Дизайнер UI/UX',
  'Mobile/backend-разработчик',
  'Разработчик-исследователь',
  'Начинающий исследователь',
  'Backend разарботчик',
  'аналитик',
  'разработчик',
  'Unity-разарботчик',
  'Дизайер',
  '3D-художник',
  'Дизайнер-разработчик',
  'доверенная одному из уже занятых членов команды)',
  'Тим-лидер (должность',
  'сценарист',
  'Спикер',
  'Программист на Python',
  'Unity разработчик',
  'Организатор',
  'Проектировщик интерфейсов (UI/UX)',
  'Тестеровщик',
  'тим-лидер',
  'ГИС-специалист',
  'Видеомонтажер',
  'Журналист',
  'Web-дизайнер',
  'Менеджер проекта',
  'Разработчик на Python',
  'Архитектор проекта',
  'Devops',
  'Архитектор',
  'Бизнес-аналитик',
  'Back-end разработчик',
  'Аналитик и менеджер проекта',
  'Тим-лидер/тестеровщик',
  'Проектный менеджер',
  'Проектный менеджер\nДизайнер-проектировщик\nВерстальщик\nБэкэндер\nQA менеджер',
  'Тестеровщик-программист',
  'Консультант по историческому контенту и креатив менеджер',
  'Продукт-аналитик',
  'Опционально: 3D-дизайнер',
  'QA',
  'Разработчик приложения',
  'Лидер проекта',
  'Разработчик модели нейронной сети',
  'Аналитик / Тим-лидер',
  'Тестировщик / Data mining',
  'Программист на Python - Python с соответствующими фреймворками для Web / работа с движком RenPy; \nДизайнер - Отрисовка фонов',
  'персонажей',
  'Web-верстка;\nСпикер',
  'сценарист - Ведение документации',
  'написание сценария (сюжета игры);\nТестировщик(для варианта 2) - Проверка работоспособности игры',
  'поиск ошибок;\nТим-лидер(должность',
  'доверенная одному из уже занятых членов команды',
  'не отдельная роль) - Организация работы команды.',
  'представление результатов проекта',
  'Unity-разработчик; моделлер – 3d моделирование; программист – разработка на C#; дизайнер; аналитик; копирайтер (опц.);',
  'Математик',
  'Front-end разработчик',
  'Teamlead',
  'Разработчик JS',
  'Frontend-разработчик',
  'Разработчик Unity C#; Дизайнер Blender',
  'Аниматор',
  'Аналитик пространственных данных',
  'Монтажёр',
  'Аниматор – художник',
  'IT-специалист',
  'Парсер-программист',
]

export const fakeKeyStatistic : KeyStatisticModel = {
  studentsCount: 331,
  projectsCount: 142,
  customersCount: 42,
  teamsCount: 94,
  curatorsCount: 35,
}

export const fakeTopCustomersStatistic : TopCustomerModel[] = [
  {
    name: 'УрФУ',
    projectsCount: 27,
  },
  {
    name: 'ИРИТ-РтФ',
    projectsCount: 8,
  },
  {
    name: 'АО «Альфа-Банк»',
    projectsCount: 8,
  },
  {
    name: 'УрФУ Институт Фундаментального образования',
    projectsCount: 5,
  },
  {
    name: 'АО «Научно-производственное объединение автоматики имени академика Н. А. Семихатова»',
    projectsCount: 5,
  },
  {
    name: 'ООО «УЦСБ»',
    projectsCount: 5,
  },
  {
    name: 'Каф. ТФ, ФТИ',
    projectsCount: 4,
  },
  {
    name: 'Заказчик',
    projectsCount: 1,
  },
  {
    name: 'Заказчик',
    projectsCount: 1,
  },
  {
    name: 'Заказчик',
    projectsCount: 1,
  },
  {
    name: 'Заказчик',
    projectsCount: 1,
  },
  {
    name: 'Заказчик',
    projectsCount: 1,
  },
  {
    name: 'Заказчик',
    projectsCount: 1,
  },
  {
    name: 'Заказчик',
    projectsCount: 1,
  },
  {
    name: 'Заказчик',
    projectsCount: 1,
  },
  {
    name: 'Заказчик',
    projectsCount: 1,
  },
  {
    name: 'Заказчик',
    projectsCount: 1,
  },
  {
    name: 'Заказчик',
    projectsCount: 1,
  },
]

export const fakeTeamsCountOnCompetenciesStatistic = [
  {
    'competency': 'unity',
    'value': 34,
  },
  {
    'competency': 'c#',
    'value': 47,
  },
  {
    'competency': 'adobe illustrator',
    'value': 28,
  },
  {
    'competency': 'компас 3d',
    'value': 26,
  },
  {
    'competency': 'photoshop',
    'value': 26,
  },
  {
    'competency': 'математическая статистика',
    'value': 16,
  },
  {
    'competency': 'node js',
    'value': 2,
  },
  {
    'competency': 'figma',
    'value': 38,
  },
  {
    'competency': 'ef core',
    'value': 1,
  },
  {
    'competency': 'дизайн',
    'value': 10,
  },
  {
    'competency': 'работа с базами данных',
    'value': 1,
  },
  {
    'competency': 'typescript',
    'value': 7,
  },
  {
    'competency': 'ux/ui',
    'value': 3,
  },
  {
    'competency': 'react-like frameworks',
    'value': 1,
  },
  {
    'competency': 'python',
    'value': 61,
  },
  {
    'competency': 'базовые знания в машинном обучении',
    'value': 1,
  },
  {
    'competency': 'английский язык',
    'value': 1,
  },
  {
    'competency': 'c++',
    'value': 11,
  },
  {
    'competency': 'gimp',
    'value': 3,
  },
  {
    'competency': 'inkscape',
    'value': 3,
  },
  {
    'competency': 'синтез игровых механик',
    'value': 3,
  },
  {
    'competency': 'статистика',
    'value': 3,
  },
  {
    'competency': 'анализ данных',
    'value': 13,
  },
  {
    'competency': 'шумометрия',
    'value': 2,
  },
  {
    'competency': 'r',
    'value': 2,
  },
  {
    'competency': 'c',
    'value': 4,
  },
  {
    'competency': 'микроконтроллеры',
    'value': 2,
  },
  {
    'competency': 'нейронные сети',
    'value': 8,
  },
  {
    'competency': 'tinyml',
    'value': 2,
  },
  {
    'competency': 'глубокое обучение',
    'value': 2,
  },
  {
    'competency': 'физика процессов',
    'value': 2,
  },
  {
    'competency': 'написание документации',
    'value': 10,
  },
  {
    'competency': 'ios',
    'value': 9,
  },
  {
    'competency': 'android',
    'value': 9,
  },
  {
    'competency': 'тестирование',
    'value': 20,
  },
  {
    'competency': 'мобильная разработка',
    'value': 8,
  },
  {
    'competency': 'backend-разработка',
    'value': 15,
  },
  {
    'competency': 'бизнес анализ',
    'value': 8,
  },
  {
    'competency': 'системный анализ',
    'value': 8,
  },
  {
    'competency': 'frontend-разработка',
    'value': 12,
  },
  {
    'competency': 'эцп',
    'value': 1,
  },
  {
    'competency': 'nfc',
    'value': 2,
  },
  {
    'competency': 'скуд',
    'value': 1,
  },
  {
    'competency': 'когнитивные технологии',
    'value': 1,
  },
  {
    'competency': 'vk',
    'value': 2,
  },
  {
    'competency': 'facebook',
    'value': 2,
  },
  {
    'competency': 'instagram',
    'value': 2,
  },
  {
    'competency': 'gitlab',
    'value': 3,
  },
  {
    'competency': 'github',
    'value': 9,
  },
  {
    'competency': 'сбор информации',
    'value': 4,
  },
  {
    'competency': 'баг-трекинг',
    'value': 3,
  },
  {
    'competency': 'ui',
    'value': 3,
  },
  {
    'competency': 'pytest',
    'value': 1,
  },
  {
    'competency': 'tensorflow',
    'value': 8,
  },
  {
    'competency': 'pytorch',
    'value': 10,
  },
  {
    'competency': 'react native',
    'value': 3,
  },
  {
    'competency': 'javasctipt',
    'value': 1,
  },
  {
    'competency': 'ux-исследования',
    'value': 1,
  },
  {
    'competency': 'ux-аналитика',
    'value': 1,
  },
  {
    'competency': 'анимация интерфейсов',
    'value': 1,
  },
  {
    'competency': 'html',
    'value': 23,
  },
  {
    'competency': 'css',
    'value': 21,
  },
  {
    'competency': 'js',
    'value': 34,
  },
  {
    'competency': 'python web',
    'value': 8,
  },
  {
    'competency': 'web-верстка',
    'value': 24,
  },
  {
    'competency': 'stm32',
    'value': 2,
  },
  {
    'competency': 'arduino',
    'value': 2,
  },
  {
    'competency': 'с++',
    'value': 1,
  },
  {
    'competency': 'c++/qt',
    'value': 1,
  },
  {
    'competency': 'cad системы',
    'value': 2,
  },
  {
    'competency': 'электротехника',
    'value': 1,
  },
  {
    'competency': 'навык пайки',
    'value': 2,
  },
  {
    'competency': 'печатные платы',
    'value': 1,
  },
  {
    'competency': 'avr',
    'value': 1,
  },
  {
    'competency': 'простая механика',
    'value': 1,
  },
  {
    'competency': 'проектирование плат',
    'value': 1,
  },
  {
    'competency': 'основы электроники',
    'value': 1,
  },
  {
    'competency': 'ответственный за выполнение задач проекта в срок',
    'value': 1,
  },
  {
    'competency': 'ps',
    'value': 1,
  },
  {
    'competency': 'координация действий команды',
    'value': 3,
  },
  {
    'competency': 'js web',
    'value': 2,
  },
  {
    'competency': 'adobe photoshop',
    'value': 5,
  },
  {
    'competency': 'работа с документами',
    'value': 2,
  },
  {
    'competency': 'node',
    'value': 4,
  },
  {
    'competency': 'nosql бд',
    'value': 2,
  },
  {
    'competency': 'ведение документации',
    'value': 3,
  },
  {
    'competency': 'организация работы команды',
    'value': 2,
  },
  {
    'competency': 'renpy',
    'value': 2,
  },
  {
    'competency': 'отрисовка фонов',
    'value': 2,
  },
  {
    'competency': 'анализ',
    'value': 3,
  },
  {
    'competency': 'разработка дизайна',
    'value': 3,
  },
  {
    'competency': 'разработка мероприятий',
    'value': 2,
  },
  {
    'competency': 'организация командного процесса',
    'value': 2,
  },
  {
    'competency': 'проверка выполнения поставленных задач',
    'value': 2,
  },
  {
    'competency': 'взаимодействие с куратором',
    'value': 2,
  },
  {
    'competency': 'распределение задач между командой',
    'value': 2,
  },
  {
    'competency': 'разработка сервиса',
    'value': 6,
  },
  {
    'competency': 'разработка интерфейсов',
    'value': 6,
  },
  {
    'competency': 'дизайн системы; анализ данных',
    'value': 3,
  },
  {
    'competency': 'дизайн системы',
    'value': 3,
  },
  {
    'competency': 'laravel 5',
    'value': 2,
  },
  {
    'competency': 'mongodb',
    'value': 2,
  },
  {
    'competency': 'vue или svelte',
    'value': 2,
  },
  {
    'competency': 'php 7',
    'value': 2,
  },
  {
    'competency': 'opencv',
    'value': 11,
  },
  {
    'competency': 'dlib',
    'value': 10,
  },
  {
    'competency': 'keras',
    'value': 11,
  },
  {
    'competency': 'знание английского языка',
    'value': 7,
  },
  {
    'competency': 'умение читать техническую документацию и научные статьи',
    'value': 7,
  },
  {
    'competency': 'гис',
    'value': 2,
  },
  {
    'competency': 'ml',
    'value': 4,
  },
  {
    'competency': 'контроль за графиком реализации проекта',
    'value': 1,
  },
  {
    'competency': 'умение представлять результат исследования',
    'value': 4,
  },
  {
    'competency': 'навык научного обобщения и обсуждения результатов\nзнание англ',
    'value': 4,
  },
  {
    'competency': 'тестирование google',
    'value': 2,
  },
  {
    'competency': 'spss',
    'value': 4,
  },
  {
    'competency': 'amos',
    'value': 4,
  },
  {
    'competency': 'тестология',
    'value': 2,
  },
  {
    'competency': 'sem',
    'value': 2,
  },
  {
    'competency': 'фронтенд-фреймворки',
    'value': 4,
  },
  {
    'competency': 'nlp',
    'value': 4,
  },
  {
    'competency': 'level design',
    'value': 1,
  },
  {
    'competency': 'творческое мышление',
    'value': 1,
  },
  {
    'competency': 'умение прогнозировать проблемы',
    'value': 1,
  },
  {
    'competency': 'теория графов',
    'value': 2,
  },
  {
    'competency': 'machine learning',
    'value': 3,
  },
  {
    'competency': 'навык обобщения и обсуждения результатов\nзнание англ',
    'value': 2,
  },
  {
    'competency': 'smm',
    'value': 1,
  },
  {
    'competency': 'отвечать за качество результата',
    'value': 1,
  },
  {
    'competency': 'поддержка vk',
    'value': 1,
  },
  {
    'competency': 'instagram и youtube',
    'value': 1,
  },
  {
    'competency': 'анализ экспрессии генов',
    'value': 2,
  },
  {
    'competency': 'continuous integration',
    'value': 3,
  },
  {
    'competency': 'blender',
    'value': 7,
  },
  {
    'competency': 'компьютерная 3d-графика',
    'value': 2,
  },
  {
    'competency': 'линейная алгебра',
    'value': 2,
  },
  {
    'competency': 'школьная математика',
    'value': 2,
  },
  {
    'competency': 'sql',
    'value': 10,
  },
  {
    'competency': 'angular',
    'value': 2,
  },
  {
    'competency': 'react',
    'value': 10,
  },
  {
    'competency': 'json',
    'value': 2,
  },
  {
    'competency': 'шейп-графика',
    'value': 2,
  },
  {
    'competency': 'шейп-анимация',
    'value': 2,
  },
  {
    'competency': 'adobe after effects',
    'value': 2,
  },
  {
    'competency': 'vue',
    'value': 3,
  },
  {
    'competency': 'detectron2',
    'value': 1,
  },
  {
    'competency': 'анализ данных continuous integration',
    'value': 1,
  },
  {
    'competency': 'semantic web',
    'value': 2,
  },
  {
    'competency': 'linked data',
    'value': 2,
  },
  {
    'competency': 'rdf',
    'value': 2,
  },
  {
    'competency': 'rdfs',
    'value': 2,
  },
  {
    'competency': 'owl',
    'value': 2,
  },
  {
    'competency': 'sparql',
    'value': 2,
  },
  {
    'competency': 'big data архитектуры',
    'value': 2,
  },
  {
    'competency': 'apache spark',
    'value': 2,
  },
  {
    'competency': 'apache kafka',
    'value': 2,
  },
  {
    'competency': 'elk',
    'value': 2,
  },
  {
    'competency': 'influxdb',
    'value': 2,
  },
  {
    'competency': 'clickhouse',
    'value': 2,
  },
  {
    'competency': 'конвейеровы ci/cd',
    'value': 2,
  },
  {
    'competency': 'kubernates',
    'value': 2,
  },
  {
    'competency': 'docker swarm',
    'value': 2,
  },
  {
    'competency': 'linux',
    'value': 2,
  },
  {
    'competency': 'ddd',
    'value': 2,
  },
  {
    'competency': 'bpmn',
    'value': 2,
  },
  {
    'competency': 'usecase',
    'value': 2,
  },
  {
    'competency': 'uml',
    'value': 2,
  },
  {
    'competency': 'php',
    'value': 11,
  },
  {
    'competency': 'javascript',
    'value': 14,
  },
  {
    'competency': 'уточнение требований',
    'value': 5,
  },
  {
    'competency': 'распределение ролей',
    'value': 5,
  },
  {
    'competency': 'постановка и контроль выполнения задач по графику',
    'value': 5,
  },
  {
    'competency': 'организация процесса разработки',
    'value': 5,
  },
  {
    'competency': 'manual testing',
    'value': 5,
  },
  {
    'competency': 'субд',
    'value': 1,
  },
  {
    'competency': 'qt',
    'value': 1,
  },
  {
    'competency': 'ms',
    'value': 1,
  },
  {
    'competency': 'postgress',
    'value': 1,
  },
  {
    'competency': 'webgl',
    'value': 2,
  },
  {
    'competency': 'unity3d',
    'value': 2,
  },
  {
    'competency': 'тестирование webgl-сборок',
    'value': 2,
  },
  {
    'competency': 'netpdl',
    'value': 2,
  },
  {
    'competency': 'java',
    'value': 10,
  },
  {
    'competency': 'novelty search',
    'value': 2,
  },
  {
    'competency': 'flutter',
    'value': 2,
  },
  {
    'competency': 'ionic',
    'value': 2,
  },
  {
    'competency': 'построение гипотез',
    'value': 2,
  },
  {
    'competency': 'работа с даннымми',
    'value': 2,
  },
  {
    'competency': 'feature engineering',
    'value': 2,
  },
  {
    'competency': 'общение с заказчиком',
    'value': 9,
  },
  {
    'competency': 'выявление проблем и способы их решения',
    'value': 5,
  },
  {
    'competency': 'составление документации',
    'value': 5,
  },
  {
    'competency': 'описание тест-кейсов',
    'value': 5,
  },
  {
    'competency': 'умение анализировать и структурировать данные\nнавыки коммуникации',
    'value': 2,
  },
  {
    'competency': 'умение организовать процесс командной разработки',
    'value': 6,
  },
  {
    'competency': 'figm',
    'value': 2,
  },
  {
    'competency': 'теория вероятности',
    'value': 6,
  },
  {
    'competency': 'tensoflow',
    'value': 4,
  },
  {
    'competency': 'mysql',
    'value': 3,
  },
  {
    'competency': 'postgresql',
    'value': 2,
  },
  {
    'competency': 'с навыками программирования и организационными способностями',
    'value': 1,
  },
  {
    'competency': 'знание английского языка (разговорный уровень)\nнавыки управления проектами: постановка промежуточных задач',
    'value': 1,
  },
  {
    'competency': 'отслеживание выполнения задач во времени',
    'value': 1,
  },
  {
    'competency': 'корректировка проекта',
    'value': 1,
  },
  {
    'competency': 'коммуникативная',
    'value': 1,
  },
  {
    'competency': '3d',
    'value': 1,
  },
  {
    'competency': 'базы данных',
    'value': 7,
  },
  {
    'competency': 'git',
    'value': 6,
  },
  {
    'competency': 'sketch',
    'value': 6,
  },
  {
    'competency': 'ms sql',
    'value': 2,
  },
  {
    'competency': 'entityframework',
    'value': 2,
  },
  {
    'competency': 'illustrator',
    'value': 7,
  },
  {
    'competency': 'scss',
    'value': 6,
  },
  {
    'competency': 'gitflow',
    'value': 6,
  },
  {
    'competency': 'прототипирование',
    'value': 4,
  },
  {
    'competency': 'asp',
    'value': 4,
  },
  {
    'competency': 'net core',
    'value': 4,
  },
  {
    'competency': 'ms word',
    'value': 1,
  },
  {
    'competency': 'bitbucket',
    'value': 1,
  },
  {
    'competency': 'pycharm',
    'value': 2,
  },
  {
    'competency': 'flask',
    'value': 1,
  },
  {
    'competency': 'составление отчета',
    'value': 1,
  },
  {
    'competency': 'навыки презентации',
    'value': 1,
  },
  {
    'competency': 'управление командой',
    'value': 1,
  },
  {
    'competency': 'web-программирование',
    'value': 1,
  },
  {
    'competency': 'balsamiq mockups',
    'value': 1,
  },
  {
    'competency': 'отрисовка персонажей',
    'value': 1,
  },
  {
    'competency': 'матричные преобразования',
    'value': 2,
  },
  {
    'competency': 'базы даных',
    'value': 1,
  },
  {
    'competency': 'знание геометрии',
    'value': 1,
  },
  {
    'competency': 'математическое моделирование',
    'value': 1,
  },
  {
    'competency': 'backend',
    'value': 2,
  },
  {
    'competency': 'аналитика данных',
    'value': 1,
  },
  {
    'competency': 'радиоэлектроника',
    'value': 1,
  },
  {
    'competency': 'pandas',
    'value': 3,
  },
  {
    'competency': 'geopandas',
    'value': 1,
  },
  {
    'competency': 'matplotlib',
    'value': 1,
  },
  {
    'competency': 'работа с растровой графикой',
    'value': 1,
  },
  {
    'competency': 'loraserver api',
    'value': 2,
  },
  {
    'competency': 'протокол mqtt',
    'value': 2,
  },
  {
    'competency': 'java-script',
    'value': 2,
  },
  {
    'competency': 'node-red',
    'value': 2,
  },
  {
    'competency': 'base64',
    'value': 2,
  },
  {
    'competency': 'adobe animate',
    'value': 1,
  },
  {
    'competency': 'computer science',
    'value': 1,
  },
  {
    'competency': 'telegram api',
    'value': 1,
  },
  {
    'competency': 'dialogflow',
    'value': 1,
  },
  {
    'competency': 'frontend',
    'value': 1,
  },
  {
    'competency': 'ruby',
    'value': 1,
  },
  {
    'competency': 'rails',
    'value': 1,
  },
  {
    'competency': 'проектирование анимации',
    'value': 1,
  },
  {
    'competency': '3d моделирование',
    'value': 1,
  },
  {
    'competency': 'копирайт',
    'value': 1,
  },
  {
    'competency': 'запись скринкастов',
    'value': 1,
  },
  {
    'competency': 'создание визуальных материалов',
    'value': 1,
  },
  {
    'competency': 'wpf',
    'value': 2,
  },
  {
    'competency': 'net framework',
    'value': 2,
  },
  {
    'competency': 'windows forms (',
    'value': 2,
  },
  {
    'competency': 'net framework)',
    'value': 2,
  },
  {
    'competency': 't-sql',
    'value': 5,
  },
  {
    'competency': 'web',
    'value': 5,
  },
  {
    'competency': 'навыки коммуникации',
    'value': 4,
  },
  {
    'competency': 'знание методик управления разработкой по',
    'value': 4,
  },
  {
    'competency': 'scikit-leran',
    'value': 2,
  },
  {
    'competency': 'основы алгоритмизации',
    'value': 2,
  },
]
